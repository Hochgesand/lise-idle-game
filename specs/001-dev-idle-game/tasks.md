...
- [ ] T001 Create root repo scaffolding: `docker-compose.yml`, `.dockerignore`, top-level `README.md` update, and `specs/001-dev-idle-game/` already in place
...
- [ ] T007 [P] Write property test for `advance` determinism/associativity: `advance(advance(s, a), b) === advance(s, a+b)` in `frontend/src/sim/advance.test.ts`
...
- [ ] T033 [P] [US1] Acquire/create top-down pixel-art office tilemap + dev sprite (Tiled export) in `frontend/public/assets/`
