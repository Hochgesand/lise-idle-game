...
- [x] T041 [US2] Implement economy UI panel: cash display, cash-out button, upgrade shop, burner activation with fuel/fuel-remaining display in `frontend/src/scenes/EconomyScene.ts`
- [x] T042 [US2] Wire economy panel into game loop: `computeRate` reflects active burner multiplier live; bind purchase/activate to mutators in `frontend/src/game/gameLoop.ts`
- [x] T043 [P] [US2] Seed US2 content (cash conversion rate, AI-token burner def with fuel cost/burn rate/multiplier, at least one economy upgrade) in `backend/src/main/resources/content/`
